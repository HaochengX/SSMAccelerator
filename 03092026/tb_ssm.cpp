...
// ============================================================
#ifndef TB_BUILD_TAG
#define TB_BUILD_TAG "tb_ssmu.cpp UPDATED_FOR_SSMU_NO_WBC + WSCALE_LOAD(PRESENCE) + COSIM_TOP_CALL_SSMU_STACK64 + CS_COMPARE + H1_DDR_FALLBACK + INTERLEAVE + v2_LOWRANK_XBC_BCX_ORDER TAG_2026-03-05_v3"
#endif
...
// ============================================================
// ✅ COSIM symbol fix: v2 LOW-RANK signature — ensure SSMU_STACK64 is visible
// ============================================================
extern "C" void SSMU_STACK64(
    ...
    const W_VEC              W_in_2[SSMU_RANK_T][CIN_T],// ★ v2 low-rank
...
int main() {
    ...
    static W_VEC W_in_2[SSMU_RANK_T][CIN_T];                // ★ v2 low-rank
    ...
    static DTYPE_VEC conv_state_out_ref[CONV_K-1]; // optional
    ...
    if (!load_w_mat<SSMU_RANK_T, CIN_T>    (P0("W_in_2_f32.raw.bin"),  W_in_2,  "W_in_2"))  return 1;  // ★ v2
    ...
    // ============================================================
    // Multi-step inference loop (Mamba-style cache update)
    // ============================================================
    bool all_pass = true;
    ...
    for (int s=0; s<steps; ++s) {
        ...
        // kernel/x/ref may have step-specific files (optional)
        ...
        // Optional: conv_state_out_ref (for debugging + compare if file exists)
        ...
        // Optional override cache for this step:
        // ONLY override if a *real step file* exists (filename contains _s{step}.raw.bin)
        {
        ...
        int rc = run_dut_once(
            ...
            W_in_1, W_in_2, W_delta, W_out_A, W_out_B, D_diag,
        ...
        // conv_state_out compare only if we have a ref file
        float diff_cs = 0.0f;
        if (have_cs_ref) {
            diff_cs = max_abs_diff_vec_tokens(conv_state_out_dut, conv_state_out_ref, CONV_K-1);
        }

        // Print with hard fences, token-selectable
        if (print_tok >= 0) {
        ...
        // ---- update "cache" for next step (Mamba-style inference cache) ----
        // NOTE: h1_dut is valid even when H1 stream off (DDR fallback)
    ...
    if (all_pass) {
        ...
        std::printf("          (If you enabled approx math / changed quant path, adjust TB_TOL_OUT / TB_TOL_H1 / TB_TOL_CS)\n");
        std::fflush(stdout);
        return 2;
    }
}