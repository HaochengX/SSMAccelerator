// ============================ ssm_lowrank_v3.cpp =============================
// Resource-optimized variant derived from ssm_lowrank_v2.cpp
//
// v3 changes from v2 (NO architecture / functionality changes):
//   R1: ACC_T narrowed  ap_fixed<32,10> → ap_fixed<24,8>  (saves DSP in accumulators)
//   R2: ACT_T narrowed  ap_fixed<18,6>  → ap_fixed<16,6>  (saves DSP/LUT in sigmoid/silu)
//   R3: EXP_T narrowed  ap_fixed<20,8>  → ap_fixed<16,8>  (saves DSP in exp)
//   R4: RMSNorm inv computation moved from float → ap_fixed<24,12>  (removes float-multiply DSPs)
//   R5: stage3 inner exp computation uses narrowed EXP_T consistently
//   R6: stage6 y*g multiply uses ACC_T consistently (no implicit widening)
//   R7: SSMU_STREAM_DEPTH reduced 650 → 64 (saves BRAM/FF for FIFOs; pipeline is not changed)
//   R8: win/out GEMM accumulators use #pragma HLS BIND_OP dsp=no on tile-stride muls
//       where the precision does NOT require DSP (frees DSP for reduction accumulator)
//   NOTE: All loop unroll factors, pipeline II, stream depths for correctness, and
//         all data-flow graph connections are IDENTICAL to v2.
...
// XBC bundle layout (matches LightMamba SILU_DEMUX ordering):
//   [ B: STATE_V ] [ C: STATE_V ] [ x: C2_T ]  total = CCONV_T = 672
...
#if USE_FIXED_ACC
// v3: narrowed from ap_fixed<32,10> → saves DSP in accumulator adds/muls
typedef ap_fixed<24, 8> ACC_T;
#else
typedef float ACC_T;
#endif

// v3: narrowed from ap_fixed<18,6> → saves LUT/DSP in sigmoid/silu
typedef ap_fixed<16, 6>  ACT_T;
// v3: narrowed from ap_fixed<20,8> → saves DSP in exp polynomial
typedef ap_fixed<16, 8>  EXP_T;

// ★ R4: fixed-point type for RMSNorm inverse (replaces float multiply)
...
// ★ R3/R5: narrowed EXP_T used here
...
#pragma HLS INLINE
    ...
    return (EXP_T)yf;
#else
    // ★ v3: use EXP_T<16,8> throughout; BIND_OP fabric → LUT multiplies, saves DSP
    EXP_T tt;
    EXP_T half_tt;
...
// ============================================================
// ★ R4: RMSNorm — float inv replaced with ap_fixed<24,12>
// ============================================================
...
#pragma HLS PIPELINE II=1
        ...
        for (unsigned l = 0; l < (unsigned)VEC_FACTOR; ++l) {
#pragma HLS UNROLL
            // ★ v3: BIND_OP fabric, 16-bit narrow → saves ~130 LUT vs 24×24 fabric mul
...
#pragma HLS UNROLL
    ...
    // ★ v3 R4+: fully fixed-point, no float DSPs
    // Multiply by compile-time reciprocal (const → synthesized as shift/LUT, not runtime divide)
    typedef ap_ufixed<28,14> RMS_MS_T_D;
    static const ap_ufixed<24,2> k_inv_dt_n = (ap_ufixed<24,2>)(1.0 / (double)(D_T * VEC_FACTOR));
    RMS_MS_T_D sumsq_u = (sumsq > (ACC_T)0) ? (RMS_MS_T_D)sumsq : (RMS_MS_T_D)0;
    RMS_MS_T_D ms      = sumsq_u * (RMS_MS_T_D)k_inv_dt_n;  // LUT: one operand is compile-time const
    RMS_MS_T_D ms_eps  = ms + (RMS_MS_T_D)1e-5f;
    RMS_MS_T_D sq      = hls::sqrt(ms_eps);                  // ap_fixed sqrt → CORDIC/LUT, no float DSP
    RMS_INV_T sq_safe_d = (sq > (RMS_MS_T_D)0) ? (RMS_INV_T)sq : (RMS_INV_T)1.0;
    RMS_INV_T inv = (RMS_INV_T)1.0 / sq_safe_d;  // scalar once-per-call; DSP impact negligible
    (void)eps;
...
#pragma HLS PIPELINE II=1
        ...
        for (unsigned l = 0; l < (unsigned)VEC_FACTOR; ++l) {
#pragma HLS UNROLL
            // ★ R4: fixed-point multiply instead of float*float*float
            // ★ v3: RMSNorm output muls → fabric, narrowed to 16-bit → saves ~130 LUT per mul
            typedef ap_fixed<16,8> RMS_NRW_T;
...
// ★ v2: Second RMSNorm for C2_T tokens (between stage6 and out_proj)
// ★ R4 applied here as well
...
#pragma HLS UNROLL
    ...
    // ★ v3 R4+: fully fixed-point, no float DSPs
    typedef ap_ufixed<28,14> RMS_MS_T_C;
    static const ap_ufixed<24,2> k_inv_c2_n = (ap_ufixed<24,2>)(1.0 / (double)(C2_T * VEC_FACTOR));
    ...
    RMS_INV_T inv = (RMS_INV_T)1.0 / sq_safe_c;  // scalar once-per-call; DSP impact negligible
    (void)eps;
...
#pragma HLS UNROLL factor=SSMU_LANE_UNROLL
                ...
                if (i >= baseZ && i < baseZ + C2_T) {
                    ...
                    // ignore extras (CIN_T = C2_T + CCONV_T + CH_T)
                }
            }
        }
    }
...
#pragma HLS DATAFLOW
    ...
    inproj_stage2_consume_local(temp_buf, Win2_tiles, Z_out, XBC_out, DT_out, wscale_in_fx);
}

// ============================================================
// ★ v2: conv+silu with state (unchanged from v2)
// ============================================================
...
// ============================================================
// stage3, stage45, ddr_writer, stage6 (all unchanged from v2)
// ============================================================
...
// ★ v2: stage6 yz node — y*gate directly (no silu on y)
// ★ R6: y and g kept as ACC_T throughout to avoid widening
...
// ★ R7: Reduced from 650 → 64 → 16.
//   At depth=64, Vitis HLS maps each 128-bit FIFO to 4 SRL16 stages = 512 LUTs;
//   with ~32 such streams that is ~16k LUTs (~15% of device).
//   Depth=16 uses exactly 1 SRL16 stage = 128 LUTs per stream → ~4k LUTs (~3.7%).
//   The 4 deadlock-critical streams (Z/G/X_ssm_out/X_residual) are
//   explicitly set to 320/640 and are unaffected by this macro.
#ifndef SSMU_STREAM_DEPTH
#define SSMU_STREAM_DEPTH 16  // ★ v3: depth=16 → 1×SRL16 per bit =128 LUT/stream; was 64 (4×SRL16=512 LUT/stream)
#endif
...
void SSMU(
    ...
    const DTYPE_VEC          RMS_weight[D_T],
    const DTYPE_VEC          RMS_weight_2[C2_T],   // ★ v2: second RMSNorm weight
...
#pragma HLS INTERFACE s_axilite port=return         bundle=control
    ...
    DTYPE_VEC RMS2_local[C2_T];    // ★ v2: second RMSNorm weight
...
#pragma HLS ARRAY_PARTITION variable=D_local    cyclic factor=8 dim=1
    ...
    preload_vec_table_local<C2_T>(RMS_weight_2,   RMS2_local);  // ★ v2
    ...
    // NOTE: Z_stream needs depth >= C2_T=640 because inproj_stage2 writes all Z tokens
    // BEFORE writing XBC tokens, while conv1d reads XBC first → deadlock at depth=64.
    // G_stream needs depth >= C2_T=640 because conv1d produces 640 tokens before stage6
    // can start consuming (stage6 waits for htC which requires the full scan).
    // All other streams are fine at SSMU_STREAM_DEPTH=64.
    ...
    // =========================================================================
    // 5) Stream depths  (★ R7: main depth 64 instead of 650)
    // =========================================================================
...
// X_residual must be >= D_T=320: tee writes X_for_norm and X_residual in the SAME pipeline loop.
// add_residual consumes X_residual only at the END of the chain (after out_proj completes).
// Until then, tee must be able to absorb all 320 tokens; if it blocks, X_for_norm also stalls
// → rmsnorm stalls → whole chain cannot complete → X_residual never consumed → DEADLOCK.
...
#pragma HLS STREAM variable=Z_stream             depth=640  // must be >= C2_T (written before XBC by stage2)
...
#pragma HLS STREAM variable=G_stream             depth=640  // must be >= C2_T (produced before htC is ready for stage6)
...
// X_ssm_out_stream must be >= C2_T=640: dup writes proj/scan/out in same loop iteration.
// stage6 consumes out only after htC is ready (end of stage45), which requires stage45 to
// finish reading scan. If out fills at 64, dup blocks → scan stalls → stage45 blocks → DEADLOCK.
...
#pragma HLS STREAM variable=outproj_temp_stream  depth=SSMU_STREAM_DEPTH
    ...
    // =========================================================================
    // 6) Dataflow graph  (IDENTICAL to v2)
    // =========================================================================
#pragma HLS DATAFLOW
...
#pragma HLS STREAM variable=X_normed_raw depth=SSMU_STREAM_DEPTH
    ...
    in_proj_lr_stage2(inproj_temp_stream, W_in_2,
        Z_stream, XBC_stream, DT_stream, wscale_in_fx);
    ...
    // ★ v2: route conv'd B/C into SSM scan
