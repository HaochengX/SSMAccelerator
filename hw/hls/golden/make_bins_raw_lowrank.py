...
Changes from make_bins_raw.py:
    Input projection split (scalar 10576):
        [5120] + [5120] + [80+128+128]
    where only middle 5120 uses low-rank:
        W_in_1[D_T][RANK_T] + W_in_2[RANK_T][C2_T]
    and non-low-rank part uses:
        W_in_nonlr[D_T][(C2_T+CH_T+2*STATE_V)]
...
comparison is exact for this split architecture.
"""
...
def q412_roundtrip_f32(arr_f32: np.ndarray) -> np.ndarray:
    q = f32_to_i16_q412(arr_f32)
    return (q.astype(np.float32) / np.float32(Q_SCALE)).astype(np.float32)

def quant_ap_fixed(arr: np.ndarray, total_bits: int, int_bits: int) -> np.ndarray:
    x = np.asarray(arr, dtype=np.float32)
    frac_bits = total_bits - int_bits
    scale = np.float32(1 << frac_bits)
    min_v = np.float32(-(1 << (int_bits - 1)))
    max_v = np.float32((1 << (int_bits - 1)) - (1.0 / float(1 << frac_bits)))
    q = np.round(x * scale).astype(np.float32) / scale
    q = np.clip(q, min_v, max_v).astype(np.float32)
    return q

def q_dtype(x: np.ndarray) -> np.ndarray:
    return quant_ap_fixed(x, DTYPE_TOTAL_BITS, DTYPE_INT_BITS)

def q_act(x: np.ndarray) -> np.ndarray:
    return quant_ap_fixed(x, ACT_TOTAL_BITS, ACT_INT_BITS)

def q_rms_nrw(x: np.ndarray) -> np.ndarray:
    return quant_ap_fixed(x, RMS_NRW_TOTAL_BITS, RMS_NRW_INT_BITS)
...
def sigmoid_fx(x, accurate):
    x = q_act(np.asarray(x, dtype=np.float32))
    if accurate:
        y = (1.0 / (1.0 + np.exp(-x, dtype=np.float32))).astype(np.float32)
        return q_act(y)
...
def silu_fx(x, accurate):
    x = q_dtype(np.asarray(x, dtype=np.float32))
    y = (q_act(x) * sigmoid_fx(x, accurate)).astype(np.float32)
    return q_dtype(y)
...
def rmsnorm_vec_tokens_c2t(x_tokens, rms_weight_tokens, eps=1e-5):
    ...
    x_nrw = q_rms_nrw(x)
    vsq_n = q_rms_nrw(x_nrw * x_nrw)
    ms = np.mean(vsq_n, dtype=np.float32)
    inv = 1.0 / np.sqrt(ms + eps, dtype=np.float32)

    inv_nrw = q_rms_nrw(inv)
    rms_xi = q_rms_nrw(x_nrw * inv_nrw)
    y = q_rms_nrw(rms_xi * q_rms_nrw(w))
    return q_dtype(y)
...
def main(out_dir="bins_raw", ssmu_h="SSMU.h", seed=0, rank=1024, also_write_f32=False,
    ...
    #   non-low-rank: [Z: C2_T] + [DT: CH_T] + [B: STATE_V] + [C: STATE_V]
    #   low-rank    : [X_mid: C2_T]
    INP_NONLR_T = C2_T + CH_T + 2 * STATE_V

    # ★ LOW-RANK / split Weights (float32)
    W_in_1  = (rng.standard_normal((D_T,    RANK_T, CP)).astype(np.float32) * 0.02)
    W_in_2  = (rng.standard_normal((RANK_T, C2_T,   CP)).astype(np.float32) * 0.02)
    W_in_nonlr = (rng.standard_normal((D_T, INP_NONLR_T, CP)).astype(np.float32) * 0.02)
    ...
    # Align golden math with DUT fixed-point path:
    # all DUT-consumed tensors are quantized to Q4.12 then dequantized to float32
    x_in          = q412_roundtrip_f32(x_in)
    h0_in         = q412_roundtrip_f32(h0_in)
    conv_state_in = q412_roundtrip_f32(conv_state_in)
    kernel_in     = q412_roundtrip_f32(kernel_in)

    A_fixed      = q412_roundtrip_f32(A_fixed)
    RMS_weight   = q412_roundtrip_f32(RMS_weight)
    RMS_weight_2 = q412_roundtrip_f32(RMS_weight_2)
    D_diag       = q412_roundtrip_f32(D_diag)

    W_in_1     = q412_roundtrip_f32(W_in_1)
    W_in_2     = q412_roundtrip_f32(W_in_2)
    W_in_nonlr = q412_roundtrip_f32(W_in_nonlr)
    W_delta    = q412_roundtrip_f32(W_delta)
    W_out_A    = q412_roundtrip_f32(W_out_A)
    W_out_B    = q412_roundtrip_f32(W_out_B)
    ...
    print(f"[PY] W_in_nonlr: {W_in_nonlr.shape}  elems={W_in_nonlr.size}")
    ...
    # Input projection split compute:
    #   low-rank middle X: X_normed @ W_in_1 @ W_in_2  -> [C2_T, CP]
    #   non-lr branch    : X_normed @ W_in_nonlr       -> [C2_T+CH_T+2*STATE_V, CP]
    X_mid = inproj_lowrank(X_normed, W_in_1, W_in_2, D_T, RANK_T, C2_T, CP)

    nlr = np.zeros((INP_NONLR_T, CP), dtype=np.float32)
    for lane in range(CP):
        nlr[:, lane] = X_normed[:, lane].astype(np.float32) @ W_in_nonlr[:, :, lane].astype(np.float32)

    Z  = nlr[0:C2_T, :]
    DT = nlr[C2_T:C2_T + CH_T, :]
    B0 = nlr[C2_T + CH_T:C2_T + CH_T + STATE_V, :]
    C0 = nlr[C2_T + CH_T + STATE_V:C2_T + CH_T + 2 * STATE_V, :]
    XBC = np.concatenate([B0, C0, X_mid], axis=0).astype(np.float32)
    assert XBC.shape[0] == CCONV_T, f"XBC tokens mismatch: {XBC.shape[0]} vs {CCONV_T}"
    ...
    # Diagnostic stage references for csim mismatch localization
    write_f32(os.path.join(out_dir, "z_f32.bin"), Z)
    write_f32(os.path.join(out_dir, "x_mid_f32.bin"), X_mid)
    write_f32(os.path.join(out_dir, "dt_f32.bin"), DT)
    write_f32(os.path.join(out_dir, "b0_f32.bin"), B0)
    write_f32(os.path.join(out_dir, "c0_f32.bin"), C0)
    write_f32(os.path.join(out_dir, "x_gate_f32.bin"), X_gate)
    write_f32(os.path.join(out_dir, "x_ssm_f32.bin"), X_ssm)
    write_f32(os.path.join(out_dir, "ssm_core_ref_f32.bin"), ssm_core_out)
    write_f32(os.path.join(out_dir, "ssm_normed_ref_f32.bin"), ssm_normed)
    write_f32(os.path.join(out_dir, "out_proj_ref_f32.bin"), out_proj_y)
