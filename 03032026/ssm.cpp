...
//
// Naming alignment w/ Scala:
// - Z_stream = raw in-proj Z (pre-activation)
// - G_stream = gate stream = silu(Z_stream)  (Scala: "z" is actually gate)
// - stage6 interface uses (x, gate) to match Scala
//
// MODE (THIS CHANGE):
// - Keep runtime w_scale_* sideband (low frequency) + protocol alignment
// - NO quant/dequant chains (remove Q8_VEC_T, SCALE streams, quant helpers, branches)
...
// ============================================================
// Latency Mode (reduce FIFO latency without changing token contract)
//   0 = original (large FIFO depths, safer for deadlock)
//   1 = low-latency (minimal FIFO depths, much smaller latency)
// ============================================================
#ifndef SSMU_LATENCY_MODE
#define SSMU_LATENCY_MODE 1
#endif

// Tile FIFO should be very small (tiles are produced/consumed immediately)
...
// ============================================================
//  This build mode: NO act-quant chain (keep macro but default 0)
// ============================================================
...
// ============================================================
// Stream-trace hook knob (does NOT change token contract)
// ============================================================
...
// NOTE: We still keep runtime sideband w_scale_* (low frequency),
// but we DO NOT do act quant/dequant chains here.
// ============================================================
...
// ============================================================
// Trace hook tee (does not change main token contract)
// ============================================================
...
// ✅ LATENCY FIX: X_buf switched to ram_2p/bram
// ============================================================
...
// Outputs:
// - G_out  : gate stream = silu(Z_in)     (Scala "z gate")
// - X_ssm  : ssm input stream = silu(conv(XBC))
// ============================================================
...
// ============================================================
// ✅ stage45 (tiled + cache + acc regs + BRAM banking)
// Token contract preserved: for each i, consume exactly C2_T from H0_in and dA_in.
// ============================================================
...
#pragma HLS PIPELINE II=1
    ...
    // Accumulation buffer: bank/partition for J_TILE=8
    DTYPE_VEC acc[C2_T];
...
#pragma HLS PIPELINE II=1
            ...
            // 1) cache H0 for this tile (avoid stream backpressure in inner compute)
...
#pragma HLS UNROLL
                ...
                else          H0_tile[jj] = dvec_zero(); // DO NOT read extra tokens
            }

            // 2) cache dA for this tile (dA_in streamed in STATE-major order)
...
#pragma HLS UNROLL
                ...
                else          dA_tile[jj] = dvec_zero(); // DO NOT read extra tokens
            }

            // 3) load acc tile into regs (reduce RAW hazard)
...
#endif
                    // state out tokens must match original (one per j per state)
...
// ============================================================
// >>> UNIFIED STAGE6: ALL PATHS GO THROUGH uD -> yz
// >>> Scala-aligned naming: stage6(x, gate)
// ============================================================
...
// Common stage6 core: x,gate are DTYPE_VEC streams
...
// ✅ FIX: remove 'read_only' (Vitis HLS 2025.1 warns unexpected pragma parameter)
#ifndef SSMU_AXI_RO_TUNE
#define SSMU_AXI_RO_TUNE max_read_burst_length=128 num_read_outstanding=32
#endif

#ifndef SSMU_DEPTH_INPROJ
#define SSMU_DEPTH_INPROJ (SSMU_D_T * SSMU_CIN_T)
#endif
#ifndef SSMU_DEPTH_DELTA
#define SSMU_DEPTH_DELTA  (SSMU_C2_T * SSMU_C2_T)
#endif
#ifndef SSMU_DEPTH_OUT
#define SSMU_DEPTH_OUT    (SSMU_D_T * SSMU_C2_T)
#endif

// latency-mode controlled large stream depths
...
#pragma HLS INTERFACE m_axi port=W_inproj    offset=slave bundle=gmemInproj depth=SSMU_DEPTH_INPROJ SSMU_AXI_RO_TUNE
#pragma HLS INTERFACE m_axi port=W_delta     offset=slave bundle=gmemDelta  depth=SSMU_DEPTH_DELTA  SSMU_AXI_RO_TUNE
#pragma HLS INTERFACE m_axi port=W_out       offset=slave bundle=gmemOut    depth=SSMU_DEPTH_OUT    SSMU_AXI_RO_TUNE
...
#pragma HLS INTERFACE s_axilite port=return         bundle=control
    ...
    DUT_PRINTF("[DUT] LATENCY_MODE=%d STREAM_DEPTH=%d TRACE_DEPTH=%d TILE_DEPTH=%d\n",
               (int)SSMU_LATENCY_MODE, (int)SSMU_STREAM_DEPTH, (int)SSMU_TRACE_DEPTH, (int)SSMU_DEPTH_TILE);
#endif
...
#pragma HLS ARRAY_PARTITION variable=D_local    cyclic factor=8 dim=1
    ...
    hls::stream<DTYPE_VEC> Z_stream("Z_stream");          // raw Z (pre-silu)
    ...
    hls::stream<DTYPE_VEC> G_stream("G_stream");          // gate = silu(Z_stream)
...
#pragma HLS STREAM variable=inproj_packed        depth=SSMU_STREAM_DEPTH
...
#pragma HLS STREAM variable=X_normed_raw depth=SSMU_STREAM_DEPTH
    ...
    // duplicate X_ssm for different uses
...
#pragma HLS STREAM variable=htC_raw depth=SSMU_STREAM_DEPTH
    ...
    // =========================================================
    // stage6 core output: ALL paths unified through uD -> yz
    // stage6(x, gate) aligned w/ Scala
    // (NO quant/dequant)
    // =========================================================
#if SSMU_ENABLE_TRACE_STREAMS
...
#pragma HLS STREAM variable=core_raw depth=SSMU_STREAM_DEPTH
    ...
    // NOTE: trace_* streams intentionally not drained here.
}

// ============================================================
// Compatibility wrapper: provide the symbol required by syn.top
// NOTE: This is a *symbol fix* only. It does NOT implement true 64-layer chaining.
// ============================================================
...
#pragma HLS INLINE off
    ...
    SSMU(kernel_in,
         A_fixed, RMS_weight, W_inproj, W_delta, W_out, D_diag,
         X_in, H0_in,
