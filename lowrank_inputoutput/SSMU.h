...
// ---------- Quant types for "方案 A" (htC/H1_state quant only) ----------
// You can switch these without touching ssm.cpp as long as vget/vset exist.
#ifndef SSMU_Q_BITS
#define SSMU_Q_BITS 16
#endif

#if (SSMU_Q_BITS == 8)
typedef ap_int<8>  DTYPE_Q;
#elif (SSMU_Q_BITS == 16)
typedef ap_int<16> DTYPE_Q;
#else
typedef ap_int<16> DTYPE_Q; // default safe
#endif

typedef hls::vector<DTYPE_Q, VEC_FACTOR> DTYPE_QVEC;

// ---------- INT8 weight path types ----------
...
//  ✅ FIX: provide overloads for BOTH DTYPE_VEC and DTYPE_QVEC
//          so ssm.cpp can vget/vset quant vectors without mismatch.
// =============================================================
...
// ---- DTYPE_QVEC (htC/H1_state quant streams) ----
static inline DTYPE_Q vget(const DTYPE_QVEC& v, unsigned idx) {
#pragma HLS INLINE
    return v[idx];
}
static inline void vset(DTYPE_QVEC& v, unsigned idx, DTYPE_Q val) {
#pragma HLS INLINE
    v[idx] = val;
}

// ---- INT8 weight vectors (only when enabled) ----
#if SSMU_USE_INT8
