...
// ✅ in_proj_pack_stream_local() / packed(): DT always emits CH_T tokens
// ✅ split_inproj_packed_local(): always consumes CH_T tokens; if DT disabled -> drop (no write)
//
// FIX #5 (latency / II improvement):
// ✅ in_proj_*: replace ACC_T acc[][] with hls::vector accv[] registers
// ✅ add DEPENDENCE inter false on accv
// ✅ pipeline jt with II=1
//
...
//
// NEW FIX #8 (THIS REQUEST):
// ✅ stage45_update_reduce_local(): H0 tile cache + acc j-tile regs + BRAM banking (cyclic=8)
// ✅ Trace hook (stream tee) that does NOT change token contract:
//    - SSMU_ENABLE_TRACE_STREAMS
//    - tee_vec_n_local(in, out_main, out_trace, n)
//
// =============================================================================================================
...
#pragma HLS INTERFACE s_axilite port=return bundle=control
    ...
    DUT_PRINTF("[DUT] TRACE_STREAMS=%d\n", (int)SSMU_ENABLE_TRACE_STREAMS);
#endif
...
#pragma HLS STREAM variable=X_normed    depth=2500
    ...
    // ---- trace streams (only exist when enabled)
#if SSMU_ENABLE_TRACE_STREAMS
...
#pragma HLS STREAM variable=trace_rms   depth=SSMU_DEPTH_TRACE
#pragma HLS STREAM variable=trace_delta depth=SSMU_DEPTH_TRACE
#pragma HLS STREAM variable=trace_htC   depth=SSMU_DEPTH_TRACE
#pragma HLS STREAM variable=trace_core  depth=SSMU_DEPTH_TRACE
#endif
...
#pragma HLS STREAM variable=X_normed_raw depth=2500
...
#pragma HLS STREAM variable=delta_for_scan_raw depth=2500
    ...
    // stage45 -> htC tee point (optional)
#if SSMU_ENABLE_TRACE_STREAMS
    hls::stream<DTYPE_VEC> htC_raw("htC_raw");
#pragma HLS STREAM variable=htC_raw depth=2500
...
#pragma HLS STREAM variable=htC_raw depth=2500
    ...
    // core -> trace_core tee point (optional)
#if SSMU_ENABLE_TRACE_STREAMS
    hls::stream<DTYPE_VEC> core_raw("core_raw");
#pragma HLS STREAM variable=core_raw depth=2500
...
#pragma HLS STREAM variable=core_raw depth=2500
    ...
    // NOTE: trace_* streams are intentionally not drained here.
    // In CSIM, you can read them from TB when SSMU_ENABLE_TRACE_STREAMS=1.
}
// ============================ end of ssm.cpp ============================
